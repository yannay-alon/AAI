# Motivation
Optimization is the process of searching for the best element. It is not clear yet what does "best" men or what "element" is, we will come to this later.
We optimize on daily basis, sometimes without even realize it. Here are a few examples:
...
2. <u>Meal Planning</u>: When preparing a meal, we might want to maximize the nutrition value of the meal and the tastiness while minimizing the cost and time spend. This could involve pretty complex factors.
3. <u>Storage</u>: When organizing our belongings at home, some of us will shove everything to a closet never to open it again, but some of us will order the closet to minimize the taken space and maximize the accessibility. 
...
How we define the optimization problem mathematically?
...
Then we define a score which determines how good a candidate solution is. This score is a function:
$$
	L \colon W \to \mathbb{R}
$$ For example, this can be the time it takes for a given route. We now want to minimize this function, which in machine learning usually called the loss function (or sometimes the risk function).
...
In this case, the set of possible solution is $W = \mathbb{R}$, and the loss function is simply the given parabola $L(w) = w^2 + 1$.
...
And in fact, we don't know (at least not yet) how to minimize a general differentiable function. Instead we are going to use an algorithm that will find a good solution, not necessarily the best one.
...
This means the set of possible solutions is the set of possible weights, or equivalently, the set of functions which are parameterized by those weights.
...
$$
So a greater deviance between the prediction and the true label will result in greater loss. This gives some smoother notion of wrong, if you are wrong by a lot it is different then being wrong by a little.
...
The gradient (a multi-dimensional derivative) gives us the direction of steepest descent.
...
# Backpropagation
Backpropagation is the algorithm that allows us to calculate the gradient of a general neural network efficiently. This algorithm heavily relies on the chain-rule for derivatives, so we will have a short reminder, but first we need to think on how the network is represented from a computational point of view.
...
![[Computational graph 1.png]]
When we want to calculate the value of the function for  specific values of $a, b$ we plug those value in and propagate them forward in the graph.
...
If we want to represent an MLP, we just stack several of this one after the other:
![[MLP.png]]

Now that we know how to build the network as a graph, and we know how to calculate the output using a forward propagation, we are left with the problem of calculating the gradient of the network.
...
![[composition.png]]
Where the squiggly lines denote some arbitrarily functions (maybe with more variables).
...
So as long as $z$ "knows" how to calculate its derivative with respect of $y$, it shouldn't care what happened before $y$.
...
Moreover, is we want to calculate the derivate at a specific value, say $x = x_0$ the chain rule becomes:
$$
...
What happens when we have multiple branches? Let's look on the following graph:
![[branching composisiotn.png]]
...
![[branching composition backprop.png]]
...
$$
	...
	\end{bmatrix}
$$
Let's look how we can calculate the gradient for the examples above:
$$
...
Here are some examples for these type of functions:
$$
	\begin{aligned}
	...
	\end{aligned}
$$
The trick for deriving the Jacobian is to look on the function $f \colon \mathbb{R}^n \to \mathbb{R}^m$ as a tuple of functions $(f_1, \dots f_m)$ with each of them being $f_i \colon \mathbb{R}^n \to \mathbb{R}$.
The Jacobian calculates the derivative of the each output with respect to each input:
$$
	J_f = \begin{bmatrix}
		\nabla^T f_1 \\
		\vdots \\
...
If we look on one of the outputs, this function looks like:
$$
	y_i((u, v)) = u_i + v_i
$$
...
The motivation and the rule of adding/multiplying according to the computational graphs works the same.
...
If we look at the graph, we see that the path from $y$ to $w_i$ and the path from $y$ to $w_j$ we see they have a lot in common. This means that the intermediate derivatives can be shared along the shared parts.
...
This is the basic of how PyTorch implement their automatic differentiation. They build the computational graph, and when we want to calculate the derivative, it runs the backpropagation algorithm from the end all the way to the start.
...
Now we will see how these two parts come together to create the basic optimization loop which is used to train deep learning models:
...
In most neural networks, you can actually run the network on multiple samples at once (this is called batching). This allows faster training (Using strong hardware like GPU and TPU) in the cost of somewhat higher memory requirements.
...
In the era of big data, datasets are huge and only grow with the years (some models are basically trained over the entire internet!).
...
In fact, the randomness in this algorithm can even improve our final result. Sometimes when the loss function is not convex, we might get stack in a local minima or in a saddle point. The randomness of $B$ slightly changes the function we minimize and can push us from such local points.

Assume the following graph represents our loss function, we might stuck at $w=0$.
...
Intuitively, imagine a ball roll down from the left side of the graph:
![[non convex.png]]
If it has enough speed by the time it gets to $w=0$, it will keep roll to the right, maybe even over the bump, instead of getting stuck.
...
There are many other options to update the momentum, some include higher order momentum (similar to acceleration in our rolling ball analogue).

One of the most popular optimizer used in deep learning is the Adam optimizer, you can read about it here: [PyTorch - Adam optimizer](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html)
## Stop Conditions
Instead of predetermine the number of epochs our algorithm runs. We might seek for an alternative stop condition.
